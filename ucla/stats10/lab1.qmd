...
This section consists of fundamental concepts in learning R
...
# vector[inidices you want to extract]
# vector[condtion]

big_vec[ c(1,5,10) ]
big_vec[ 1:10 ] # what are the first 10 elements ?

head(big_vec,10) # glimpse of first 10 elements
tail(big_vec,10) # glimpse of last 10 elements

#c() can combine same type of objects

num_vec
pemdas_vec

c(num_vec,pemdas_vec) # combined the elements of these vectors
...
```{r,echo=FALSE}
birth_data_location <- here::here("static", "data", "births.csv")
birth_data_location
```
...
```{r,echo=FALSE}
birth_dat <- read.csv(birth_data_location)
```
...
**Warning:** The above tutorial assumes the data set is a `csv` file in working condition. That is we assume the default settings for `read.csv`. In some cases we may need to change the header, specify the field separator and more. See `?read.csv` for further details and examples.
...
# Section 2 -- Summarizing and visualizing data (one variable)
...
# Section 3 - Summarizing and visualizing data (two variables)
...
**Warning:** When using `legend()` it is important that you specify the correct ordering of colors for each group, otherwise the legend would be incorrect. For example, in our first histogram we chose "lightcoral" to represent "female" and "lightblue" to represent "male" distributions.
