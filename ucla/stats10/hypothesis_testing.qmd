...
Below is a preview of `flint` alonside its dimensions
```{r}
head(flint)
dim(flint)
```
...
Our sample proportion of lead levels in `Flint` that are greater than or equal to 15PPB *(dangerous lead levels)*
...
While the above process can be tedious,it can easily be performed with a few line of codes in R :)
