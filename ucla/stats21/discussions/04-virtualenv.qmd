...
`numpy > 1.17` to proprely use `default_rng()`.
...
We will take the **project-folder approacgh** and create a virtual environment inside `random_folder`.

To do so, run the following command on the terminal insde the `random_folder` directory
...
Then the virtual environment has been successfully been activated. Now we can install
the external pacakges such as NumPy and pandas 
...
Once you execute the `deactivate` command, your command prompt will return to its normal state, indicating that you have successfully exited the virtual environment. Any interactions with Python or pip from this point onwards will be carried out using your globally configured Python environment, rather than the packages and dependencies installed in the virtual environment
