...
An `NA` is produced because are vector `v3` has three elements and we are trying to access the 4th and 6th element which do not exist. Instead use the function `seq_along(x)` which will create a regular sequence from `1:length(x)`
